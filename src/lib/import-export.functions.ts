...
export const exportLibrary = createServerFn({ method: "GET" })
  ...
  .handler(async ({ context }) => {
    ...
    return (rows ?? []).map((r: { status: string; rating: number | null; favorite: boolean; hidden: boolean; notes: string | null; created_at: string; updated_at: string; media: { title: string; media_type: string; source: string; external_id: string } | null }) => {
      const m = r.media;
      return {
        title: m?.title ?? "",
  ...
  .handler(async ({ data, context }) => {
    ...
    for (let i = 0; i < data.items.length; i += BATCH) {
      ...
      const orFilter = batch
        .map((it) => `and(media_type.eq.${it.media_type},source.eq.${it.source},external_id.eq.${it.external_id})`)
        .join(",");
      ...
      const idByKey = new Map(
        (mediaRows ?? []).map((m: { id: string; media_type: string; source: string; external_id: string }) =>
          [`${m.media_type}|${m.source}|${m.external_id}`, m.id]),
      );
