...
export const Route = createFileRoute("/_authenticated/profile")({
  head: () => ({
    meta: [
      ...
      { name: "description", content: "Your stats, favorites, and watch history." },
    ],
  }),
  ...
  return (
    <div className="space-y-8">
      ...
      {libraryEmpty ? (
        <EmptyState
          ...
          action={
            <Link
              to="/search"
              className="inline-flex items-center gap-2 rounded-lg bg-gradient-accent px-5 py-2.5 text-sm font-semibold text-white shadow-lg btn-press"
            >
            ...
            <div className="glass-strong rounded-2xl p-5">
              ...
              <div className="flex items-center gap-5">
                ...
                <div className="flex-1 space-y-1.5 min-w-0">
                  <BreakdownRow
                    label="Movies"
                    value={s?.completedMovies ?? 0}
                    total={s?.completed ?? 1}
                    color="bg-primary"
                  />
                  <BreakdownRow
                    label="TV"
                    value={s?.completedTv ?? 0}
                    total={s?.completed ?? 1}
                    color="bg-accent"
                  />
                  <BreakdownRow
                    label="Anime"
                    value={s?.completedAnime ?? 0}
                    total={s?.completed ?? 1}
                    color="bg-success"
                  />
                  <BreakdownRow
                    label="Manga"
                    value={s?.completedManga ?? 0}
                    total={s?.completed ?? 1}
                    color="bg-warning"
                  />
                </div>
      ...
      {s?.topRatings && s.topRatings.length > 0 && (
        <section>
          <SectionHeader
            className="mb-4 text-lg md:text-lg"
            title={
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-6 gap-3">
            {s.topRatings.map((r) => (
              <Link
                ...
                params={{ type: r.media_type, source: r.source, id: r.external_id }}
              ...
              return (
                ...
                >
                  ...
                  <div className="absolute bottom-0 inset-x-0 bg-gradient-to-t from-black/80 to-transparent p-2">
                    <span
                      className={cn(
                        "inline-block rounded-full border px-1.5 py-0.5 text-[8px] uppercase tracking-wider font-medium",
                        r.status === "completed"
                    ...
                    <p className="mt-0.5 line-clamp-1 text-[11px] font-medium text-white">
                      {m.title}
                    </p>
                  </div>
                </Link>
      ...
      >
        <DialogContent>
          ...
          <div className="max-h-80 space-y-3 overflow-y-auto">
            {(listMode === "followers"
              ? (followersQ.data as FollowProfile[] | undefined)
              : (followingQ.data as FollowProfile[] | undefined)
            )?.length === 0 ? (
