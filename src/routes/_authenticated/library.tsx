...
import { STATUS_COLORS, getStatusLabel, type MediaSummary, type MediaType, type WatchStatus } from "@/lib/media-types";
import { formatDistanceToNow } from "date-fns";
import { Film, Eye, BookmarkPlus, CheckCircle2, Heart, Search, ArrowUpDown, X, LayoutGrid, List, Star } from "lucide-react";
...
function Library() {
    ...
    if (sortBy !== "recent") {
      ...
      filtered = [...filtered].sort((a, b) => {
        ...
        if (sortBy === "title") {
          ...
          comparison = (a.rating ?? a.media?.vote_average ?? 0) - (b.rating ?? b.media?.vote_average ?? 0);
        } else if (sortBy === "year") {
  ...
  const items: MediaSummary[] = useMemo(() => filteredRows.map((r) => {
    const m = r.media;
    const rawSource = m?.source ?? "";
    const rawType = m?.media_type ?? "";
    return {
      external_id: m?.external_id ?? "",
      source: (rawSource === "tmdb" || rawSource === "anilist" || rawSource === "jikan" || rawSource === "kitsu" ? rawSource : "tmdb") as "tmdb" | "anilist" | "jikan" | "kitsu",
      media_type: (rawType === "movie" || rawType === "tv" || rawType === "anime" || rawType === "manga" ? rawType : "movie") as MediaType,
  ...
  }).filter((i) => i.external_id), [filteredRows]);
  ...
  return (
    <div>
      ...
      <div className="mb-6 flex flex-col sm:flex-row items-stretch sm:items-center gap-3">
        ...
        {/* View toggle */}
        <div className="flex items-center gap-1 rounded-xl border border-border/40 bg-card/40 p-1 shrink-0" role="group" aria-label="View">
        ...
        <div className="flex items-center gap-2 shrink-0">
          ...
          <Select value={sortBy} onValueChange={(val) => setSortBy(val as SortOption)}>
            <SelectTrigger aria-label="Sort by" className="w-full max-w-[180px] rounded-xl border border-border/40 bg-card/40 px-3 py-2 text-sm font-medium text-foreground focus:border-primary/50 focus:outline-none cursor-pointer">
              <SelectValue />
            </SelectTrigger>
      ...
      {q.isLoading && rows.length === 0 ? (
        view === "grid" ? <SkeletonGrid count={12} /> : (
...
function LibraryList({ rows }: { rows: Array<{
  id: string;
  status: WatchStatus;
  favorite: boolean;
  rating: number | null;
      ...
      <table className="w-full text-sm">
        <thead>
          <tr className="border-b border-border/40 text-left text-[11px] uppercase tracking-wider text-muted-foreground/60">
            ...
            <th scope="col" className="hidden px-3 py-2.5 font-semibold md:table-cell">Type</th>
            <th scope="col" className="px-3 py-2.5 font-semibold">Status</th>
            <th scope="col" className="px-3 py-2.5 font-semibold">Rating</th>
            <th scope="col" className="hidden px-3 py-2.5 font-semibold md:table-cell">Updated</th>
          </tr>
        </thead>
        <tbody>
          {rows.map((r) => {
            ...
            const mediaType = (["movie", "tv", "anime", "manga"].includes(m.media_type) ? m.media_type : "movie") as MediaType;
            const statusLabel = getStatusLabel(r.status, mediaType);
            return (
              <tr key={r.id} className="border-b border-border/20 transition-colors last:border-0 hover:bg-muted/30">
                <td className="px-4 py-2.5">
                  <Link to={LIST_LINK} params={{ type: m.media_type, source: m.source, id: m.external_id }} className="flex items-center gap-3 min-w-0 group/row">
                    <SafeImage
                      src={m.poster_url}
                      alt=""
                    ...
                    <span className="min-w-0">
                      <span className="flex items-center gap-1.5 font-medium leading-tight group-hover/row:text-primary transition-colors">
                        ...
                        {r.favorite ? <Heart className="h-3 w-3 shrink-0 fill-current text-accent" aria-label="Favorite" /> : null}
                      </span>
                      {m.release_year ? <span className="text-xs text-muted-foreground">{m.release_year}</span> : null}
                    </span>
                  </Link>
                </td>
                <td className="hidden px-3 py-2.5 capitalize text-muted-foreground md:table-cell">{mediaType}</td>
                <td className="px-3 py-2.5">
                  <span className={cn("inline-block rounded-full border px-2 py-0.5 text-[11px] font-medium", STATUS_COLORS[r.status])}>
                    {statusLabel}
                  </span>
                </td>
                ...
                <td className="hidden px-3 py-2.5 text-xs text-muted-foreground md:table-cell" title={new Date(r.updated_at).toLocaleString()}>
